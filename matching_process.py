...
"""function that does the comparation using the algo longest_common_substring"""
def match():
  ...
  def longest_common_substring(mishna, citation):
  ...
  for citation_id, citation_daf, citation_text in citations:  
       ...
       for mishna_id, mishna_daf, mishna_text in mishnayot:
         ...
         sub_s=longest_common_substring(mishna,citation)
         if sub_s >= 3: 
            index+=1
            matched_count+=1
            insert_match(index,mishna_daf,citation_daf)
            break
